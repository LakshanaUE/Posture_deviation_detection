...
def extract_features_realtime(landmarks, frame_width=640, frame_height=480):
    ...
    # ── Calculate angles ──────────────────────────────────────
    # Use PIXEL coordinates for angle display (degrees)
    l_ear_px  = (l_ear[0]*frame_width,  l_ear[1]*frame_height)
    r_ear_px  = (r_ear[0]*frame_width,  r_ear[1]*frame_height)
    l_sh_px   = (l_sh[0]*frame_width,   l_sh[1]*frame_height)
    r_sh_px   = (r_sh[0]*frame_width,   r_sh[1]*frame_height)
    l_hip_px  = (l_hip[0]*frame_width,  l_hip[1]*frame_height)
    r_hip_px  = (r_hip[0]*frame_width,  r_hip[1]*frame_height)
    sh_mid_px = (sh_mid_x*frame_width,  sh_mid_y*frame_height)
    hi_mid_px = (hi_mid_x*frame_width,  hi_mid_y*frame_height)
