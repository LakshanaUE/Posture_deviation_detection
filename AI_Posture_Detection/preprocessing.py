...
for d in [MERGED_DIR, PROCESSED_DIR, REPORTS_DIR]:
...
def extract_features_from_landmarks_df(df,
    ...
    for row_idx, row in df.iterrows():
        try:
            ...
            hi_mid_x = (l_hi_x + r_hi_x) / 2
            hi_mid_y = (l_hi_y + r_hi_y) / 2

            neck_l  = _neck_angle(l_ear_x, l_ear_y, l_sh_x, l_sh_y)
            neck_r  = _neck_angle(r_ear_x, r_ear_y, r_sh_x, r_sh_y)
            neck_a  = (neck_l + neck_r) / 2
            spine   = _torso_angle(sh_mid_x, sh_mid_y, hi_mid_x, hi_mid_y)
            sh_slp  = _slope(l_sh_x, l_sh_y, r_sh_x, r_sh_y)
            hd_tlt  = _slope(l_ear_x, l_ear_y, r_ear_x, r_ear_y)
            hip_al  = _slope(l_hi_x, l_hi_y, r_hi_x, r_hi_y)
...
def load_gym_landmarks():
    ...
    """
    path = os.path.join(MERGED_DIR, "dataset_all_points.csv")
...
def load_yoga_keypoints():
    ...
    Result: ~2633 samples.
    """
    path = os.path.join(MERGED_DIR, "yoga_keypoints.csv")
...
def load_confidence_features():
    ...
    """
    path = os.path.join(MERGED_DIR, "confidence_features.csv")
