...
def draw_hud(frame, posture_text, confidence, neck_angle, torso_angle,
             fps, bad_duration, session_stats, angles_dict, is_bad):
    h, w = frame.shape[:2]
    ...
    tc = COLOR_BAD if torso_angle >= 25.0 else (COLOR_WARN if torso_angle >= 20.0 else COLOR_GOOD)
    ...
    cv2.putText(frame, posture_text, (15,h-bar_h+32), FONT_B, 0.75, lc, 2, cv2.LINE_AA)
...
def draw_no_person(frame):
    h, w = frame.shape[:2]
    cv2.putText(frame, "No person detected", (w//2-130,h//2), FONT_B, 0.9, (100,100,100), 2, cv2.LINE_AA)
    cv2.putText(frame, "Please stand in front of the camera", (w//2-180,h//2+35), FONT, 0.6, (70,70,70), 1, cv2.LINE_AA)
...
class PostureDetector:
    ...
    def process_frame(self, pose_landmarker):
        ...
        # Alerts
        alert_fired  = self.alert_manager.update_bad_posture(bad_flag)
        ...
        draw_hud(frame, posture_text, confidence, neck_avg, torso_angle,
                 fps, bad_duration, self.session_stats, angles_dict, bad_flag)
...
if __name__ == "__main__":
    detector = PostureDetector(
        ...
        use_ml_model=True, bad_posture_threshold=3.0, alert_cooldown=6.0,
    )
